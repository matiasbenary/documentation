...
In order for a contract to be considered a FT contract it has to follow the [**NEP-141**](https://github.com/near/NEPs/tree/master/neps/nep-0141) and [**NEP-148 standards**](https://github.com/near/NEPs/tree/master/neps/nep-0148)which define the **minimum interface** required to be implemented, as well as the expected functionality.

---

## NEP-141 (Fungible Token Interface)

[NEP-141](https://github.com/near/NEPs/tree/master/neps/nep-0141) is the blueprint for all fungible tokens (e.g. stablecoins, governance tokens, etc.) on NEAR.
...
<Accordion title="ft_on_transfer">
    ...
    See a reference implementation in the [using FTs page](./ft#handling-deposits)
...
This method is used as a [callback](../../smart-contracts/anatomy/crosscontract#callback-function) to resolve the `ft_transfer_call` transaction, handling refunds if necessary.
...
[NEP-145](https://github.com/near/NEPs/blob/master/neps/nep-0145) is a standard that defines a common interface for registering users, allowing FT contracts to **charge users for the storage they use**.
...
[NEP-148](https://github.com/near/NEPs/tree/master/neps/nep-0141) is an extension to the NEP-141 standard that defines the fungible tokens **metadata**.
