...
In most blockchains, users control their accounts by holding a single [`private key`](https://en.wikipedia.org/wiki/Public-key_cryptography) (a secret only they know) and using it to sign [transactions](./transactions).
...
3. `allowance` (Optional): The **amount of NEAR** allowed to be spent on [gas](gas). If omitted, the key can consume **unlimited** gas
...
## Full-Access Keys
As the name suggests, `Full-Access` keys have full control of an account, meaning they can be used to sign [transactions](transactions) doing any action in your account's behalf:
