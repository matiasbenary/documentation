...
NEAR consists roughly of a blockchain layer and a [runtime layer](network/runtime).
...
[Transactions](transactions) and receipts are a fundamental concept in Near Protocol. Transactions represent actions requested by the
...
Similar to Ethereum, Near Protocol is an [account-based system](account-model). Which means that each blockchain user is roughly
associated with one or several accounts (there are exceptions though, when users share an account and are separated
through the [access keys](access-keys)).
...
Even though [tokens](network/tokens) is a fundamental concept of the blockchain, it is neatly encapsulated
inside the runtime layer together with the [gas](gas), [fees](gas#understanding-gas-fees), and rewards.
...
responsible for maintaining the integrity of the Near Protocol. These participants called [Validators](network/validators) are associated with the
...
- [Fees and rewards](gas) -- fees and rewards are neatly encapsulated in the runtime layer. The blockchain layer, however,
