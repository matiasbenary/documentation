...
Users interact with NEAR by creating transactions. Specifically, users use their account's [private keys](./access-keys) to sign transactions, which are then broadcasted and processed by the network.
